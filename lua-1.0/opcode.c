...
int lua_execute (Byte *pc)
{
 ...
 while (1)
 {
  switch ((OpCode)*pc++)
  {
   ...
   case PUSHNIL: tag(top++) = T_NIL;printf("PUSHNIL,"); break;
   
   case PUSH0: tag(top) = T_NUMBER; nvalue(top++) = 0;printf("PUSH0,");  break;
   case PUSH1: tag(top) = T_NUMBER; nvalue(top++) = 1;printf("PUSH1,");  break;
   case PUSH2: tag(top) = T_NUMBER; nvalue(top++) = 2;printf("PUSH2,"); break;

   case PUSHBYTE: tag(top) = T_NUMBER; nvalue(top++) = *pc++;printf("PUSHBYTE,"); break;
   
   case PUSHWORD: 
    tag(top) = T_NUMBER; nvalue(top++) = *((Word *)(pc)); pc += sizeof(Word);printf("PUSHWORD,");
   break;
   
   case PUSHFLOAT:
    tag(top) = T_NUMBER; nvalue(top++) = *((float *)(pc)); pc += sizeof(float);printf("PUSHFLOAT,");
   break;
   case PUSHSTRING:
   {printf("PUSHSTRING,");
    int w = *((Word *)(pc));
   ...
   case PUSHLOCAL0: *top++ = *(base + 0);printf("PUSHLOCAL0,");  break;
   case PUSHLOCAL1: *top++ = *(base + 1);printf("PUSHLOCAL1,");  break;
   case PUSHLOCAL2: *top++ = *(base + 2);printf("PUSHLOCAL2,");  break;
   case PUSHLOCAL3: *top++ = *(base + 3);printf("PUSHLOCAL3,");  break;
   case PUSHLOCAL4: *top++ = *(base + 4);printf("PUSHLOCAL4,");  break;
   case PUSHLOCAL5: *top++ = *(base + 5);printf("PUSHLOCAL5,");  break;
   case PUSHLOCAL6: *top++ = *(base + 6);printf("PUSHLOCAL6,");  break;
   case PUSHLOCAL7: *top++ = *(base + 7);printf("PUSHLOCAL7,");  break;
   case PUSHLOCAL8: *top++ = *(base + 8);printf("PUSHLOCAL8,");  break;
   case PUSHLOCAL9: *top++ = *(base + 9);printf("PUSHLOCAL9,");  break;
   
   case PUSHLOCAL: *top++ = *(base + (*pc++));printf("PUSHLOCAL,"); break;
   
   case PUSHGLOBAL: 
    *top++ = s_object(*((Word *)(pc))); pc += sizeof(Word);printf("PUSHGLOBAL,");
   break;
   
   case PUSHINDEXED:printf("PUSHINDEXED\n");
    --top;
   ...
   case PUSHMARK: tag(top++) = T_MARK;printf("PUSHMARK,"); break;
   
   case PUSHOBJECT: *top = *(top-3); top++;printf("PUSHOBJECT,"); break;
   
   case STORELOCAL0: *(base + 0) = *(--top);printf("STORELOCAL0,");  break;
   case STORELOCAL1: *(base + 1) = *(--top);printf("STORELOCAL1,");  break;
   case STORELOCAL2: *(base + 2) = *(--top);printf("STORELOCAL2,");  break;
   case STORELOCAL3: *(base + 3) = *(--top);printf("STORELOCAL3,");  break;
   case STORELOCAL4: *(base + 4) = *(--top);printf("STORELOCAL4,");  break;
   case STORELOCAL5: *(base + 5) = *(--top);printf("STORELOCAL5,");  break;
   case STORELOCAL6: *(base + 6) = *(--top);printf("STORELOCAL6,");  break;
   case STORELOCAL7: *(base + 7) = *(--top);printf("STORELOCAL7,");  break;
   case STORELOCAL8: *(base + 8) = *(--top);printf("STORELOCAL8,");  break;
   case STORELOCAL9: *(base + 9) = *(--top);printf("STORELOCAL9,");  break;
    
   case STORELOCAL: *(base + (*pc++)) = *(--top);printf("STORELOCAL,"); break;
   
   case STOREGLOBAL:
    s_object(*((Word *)(pc))) = *(--top); pc += sizeof(Word);printf("STOREGLOBAL,");   
   break;

   case STOREINDEXED0:printf("STOREINDEXED0\n");
   ...
   case STOREFIELD:printf("STOREFIELD\n");
    if (tag(top-3) != T_ARRAY)
    {
   ...
   case CREATEARRAY:printf("CREATEARRAY\n");
    if (tag(top-1) == T_NIL) 
     nvalue(top-1) = 101;
    else 
    {
   ...
   {printf("EQOP:"); 
    ...
    else
    {
     switch (tag(l))
     {
      case T_NIL:       tag(top-1) = T_NUMBER;printf("T_NIL\n");  break;
      case T_NUMBER:    tag(top-1) = (nvalue(l) == nvalue(r)) ? T_NUMBER : T_NIL;printf("T_NUMBER\n");  break;
      case T_ARRAY:     tag(top-1) = (avalue(l) == avalue(r)) ? T_NUMBER : T_NIL;printf("T_ARRAY\n");  break;
      case T_FUNCTION:  tag(top-1) = (bvalue(l) == bvalue(r)) ? T_NUMBER : T_NIL;printf("T_FUNCTION\n");  break;
      case T_CFUNCTION: tag(top-1) = (fvalue(l) == fvalue(r)) ? T_NUMBER : T_NIL;printf("T_CFUNCYION\n");  break;
      case T_USERDATA:  tag(top-1) = (uvalue(l) == uvalue(r)) ? T_NUMBER : T_NIL;printf("T_USERDATA\n");  break;
      case T_STRING:    tag(top-1) = (strcmp (svalue(l), svalue(r)) == 0) ? T_NUMBER : T_NIL;printf("T_STRING\n");  break;
   ...
   case JMP: pc += *((Word *)(pc)) + sizeof(Word);printf("JMP,"); break;
    
   case UPJMP: pc -= *((Word *)(pc)) - sizeof(Word);printf("UPJMP,"); break; 
